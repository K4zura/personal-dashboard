...
>
	...
	<article class="bg-card-bg border-border relative h-max w-96 rounded border-2 p-4">
		<button
		...
		<h1 class="text-text mb-4 text-xl font-bold">Add New {title}</h1>
		<form action="{action}/add" method="POST" class="grid grid-cols-2 gap-4">
			{#each fields as field}
				{#if field[1] === 'select'}
					<label for={field[0]} class="flex flex-col gap-2 {field[2]}">
						<p class="border-divider text-text-secondary border-l-2 px-1 text-xs font-semibold">
							{field[0]}
						</p>
					...
					<div class="col-span-2 flex items-end gap-2">
						<label for={field[0]} class="relative flex w-full flex-col gap-2 {field[2]}">
							...
							<input
								...
								name={field[0]}
								id={field[0]}
								class="bg-input-bg shadow-accent w-full border-none shadow ring-0"
				...
				{/if}
			{/each}
			<button class="bg-primary text-button-text col-end-3 cursor-pointer rounded px-3 py-1.5"
				>Send</button
			>
		</form>
	</article>
